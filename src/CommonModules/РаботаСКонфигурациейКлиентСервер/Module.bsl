...
//		"--user=""%1C_USER%"" --password=""%1C_PWD%""";
	...
	Если ДополнительныеПараметрыВыгрузки.ЧерезФайлСоСписком Тогда
		...
		КонецЕсли;
	ИначеЕсли НЕ ДополнительныеПараметрыВыгрузки.ПолнаяВыгрузка
	И ДополнительныеПараметрыВыгрузки.МассивМетаданных.Количество() = 0 Тогда
...
//  ПараметрыВыполнения - см. РаботаСГитомВызовСервера.ПараметрыВыполнения
//  ПараметрыКонфигурации - см. РаботаСГитомВызовСервера.ПараметрыДляРаботыСКонфигурацией
//  ИмяФайлаМетаданные - Неопределено, Строка - Путь к файлу метаданных, если нет пути - выгрузка полная
//  Рекурсивно - Булево - Рекурсивная выгрузка, не рекомендуется включать флаг при выгрузке корня
// 
// Возвращаемое значение:
//  Строка - Команда для запуска выгрузки объектов
Функция УдалитьКомандаДляЗапускаВыгрузкиОбъектовIBCMD(ПараметрыВыполнения, ПараметрыКонфигурации, ИмяФайлаМетаданные = Неопределено, Рекурсивно = Ложь) Экспорт
	
	Если ИмяФайлаМетаданные = Неопределено Тогда
	...
	СтрокаВыполнения = СтрЗаменить(СтрокаВыполнения, "%LISTFILEPATH%", ИмяФайлаМетаданные);
...
// Команда для запуска выгрузки объектов.
// TODO: Объединить с основным методом
// 
// Параметры:
//  ПараметрыВыполнения - см. РаботаСГитомВызовСервера.ПараметрыВыполнения
//  ПараметрыКонфигурации - см. РаботаСГитомВызовСервера.ПараметрыДляРаботыСКонфигурацией
