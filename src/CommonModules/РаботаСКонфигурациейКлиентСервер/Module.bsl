...
Функция КомандаДляЗапускаВыгрузкиОбъектов(ПараметрыВыполнения, ПараметрыКонфигурации, МетаданныеКВыгрузке, Рекурсивно = Ложь) Экспорт
	...
	ШаблонКоманды = """%1CBIN%\%1CVERSION%\bin\ibcmd.exe"" infobase config export " + 
		?(ЭтоРасширение, "--extension ""%EXTENSION_NAME%"" ", "") +
		"objects %R% %OBJECTS_LIST% --out=""%WORKING_DIR%\bd"" " +
...
Функция КомандаДляЗахватаОбъектовВХранилище(ПараметрыВыполнения, ПараметрыКонфигурации, ПутьКХМЛФайлу) Экспорт
...
Функция МассивМетаданныхВХМЛДляЗахвата(МассивКоммитов) Экспорт
	...
	МетаданныеКВыгрузке = РаботаСКонфигурациейВызовСервера.МассивМетаданныхПоКоммитам(МассивКоммитов);
