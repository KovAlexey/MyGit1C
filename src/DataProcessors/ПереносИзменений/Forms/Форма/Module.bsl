...
Процедура ВыгрузитьИзБазыИсточника(Команда)
	...
	ПараметрыКонфигурацииИсточник = РаботаСГитом.ПараметрыДляРаботыСКонфигурацией(БазаИсточник);
	ПараметрыКонфигурацииПриемник = РаботаСГитом.ПараметрыДляРаботыСКонфигурацией(БазаПриемник);
	
	ПараметрыКонфигурацииИсточник.ВерсияПлатформы = ПараметрыКонфигурацииПриемник.ВерсияПлатформы; // Выгружаем той же версией
	ПараметрыВыполнения = РаботаСГитом.ПараметрыВыполнения();
	...
	КомандаКВыполению = РаботаСГитомКлиентСервер.КомандаДляЗапускаВыгрузкиОбъектов(ПараметрыВыполнения, 
		ПараметрыКонфигурацииИсточник, МетаданныеКВыгрузке, Истина);
	
	ИмяФайлаЛога = ПараметрыВыполнения.РабочийКаталог + "\log.txt"; 
	РаботаСГитомКлиентСервер.ДобавитьВыводВФайл(КомандаКВыполению, ПараметрыВыполнения, ИмяФайлаЛога);
...
Функция ВыгрузитьКорень(ПараметрыКонфигурации)
	...
	КомандаКВыполнению = РаботаСГитомКлиентСервер.КомандаДляЗапускаВыгрузкиОбъектов(ПараметрыВыполнения, 
...
Функция ФайлыКЗагрузке()
	...
	|	КоммитыИзмененныеФайлы.Ссылка В(&Ссылка)";
	Запрос.УстановитьПараметр("Ссылка", Коммиты.Выгрузить().ВыгрузитьКолонку("Коммит"));
	//ФайлыКЗагрузке = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("ИмяФайла");
...
&НаСервере
Функция КомандаВыгрузкиЧерезАвтономныйСервер(База)
	
	Результат = Новый Структура;
	
	СтрокаВыгрузки = """" + Константы.КаталогПлатформ.Получить();
	...
	КонецЕсли;
	СтрокаЗапуска1С = СтрокаЗапуска1С + БазаПриемник.ВерсияПлатформы + "\bin\ibcmd.exe" + """";
	СтрокаЗапуска1С = СтрокаЗапуска1С + " infobase config export objects";
	...
	СтрокаМетаданныеКВыгрузке = СтрСоединить(МетаданныеКВыгрузке, " ");
	СтрокаЗапуска1С = СтрокаЗапуска1С + " " + СтрокаМетаданныеКВыгрузке;
	
	КаталогВыгрузки = РабочийКаталог + "\bd";
	СтрокаЗапуска1С = СтрокаЗапуска1С + " " + СтрШаблон("--out ""%1""", КаталогВыгрузки);
	СтрокаЗапуска1С = СтрокаЗапуска1С + " " + СтрШаблон("--db-server=""%1"" --dbms=MSSQLServer --db-name=""%2"" --db-user=""%3""",
		База.СерверСКЛ.Сервер, База.СерверСКЛ.Наименование, База.СерверСКЛ.Логин);
		
	Результат.Вставить("Команда", СтрокаЗапуска1С);
	Результат.Вставить("Папка", КаталогВыгрузки);
...
Функция ПутьEDTВПутьКонфигуратора(ОтносительныйПуть)
	...
	ИначеЕсли РазобранныйПуть.Количество() > 3 Тогда
		...
		Если РазобранныйПуть[2] = "Forms" Тогда
			...
			//	//Модуль формы
			//	
			//	Для Сч = 0 По РазобранныйПуть.ВГраница() - 1 Цикл
			//		Результат = Результат + "/" + РазобранныйПуть[Сч];
			//	КонецЦикла;
			//	//Результат = Результат + "/Ext/Form";
			//	//Результат = Результат + "/" + РазобранныйПуть[РазобранныйПуть.ВГраница()];
			//ИначеЕсли Прав(РазобранныйПуть[РазобранныйПуть.ВГраница()], 5) = ".form" Тогда
			//	Для Сч = 0 По РазобранныйПуть.ВГраница() - 2 Цикл
			//		Результат = Результат + "/" + РазобранныйПуть[Сч];
			//	КонецЦикла;
			//	Результат = Результат + "/" + СтрШаблон("%1.xml", РазобранныйПуть[Граница - 1]);
...
&НаСервере
Функция СвернутьМассивНеоптимально(Массив)
...
Процедура ЗагрузитьОбъекты(Команда)
	...
	Для Сч = 0 По ФайлыКЗагрузкеСтроки.ВГраница() Цикл
		...
		Если Не Файл.Существует() Тогда
			Сообщить(Файл.ПолноеИмя + " Не существует!");
			Продолжить;
		КонецЕсли;
		ИтоговыйСписок.Добавить(КаталогБД + СтрЗаменить(ОтносительныйПутьКФайлу, "/", "\"));
		
		Если Лев(ОтносительныйПутьКФайлу, 1) = "/" Тогда
			ОтносительныйПутьКФайлу = Сред(ОтносительныйПутьКФайлу, 2);
		КонецЕсли;
		
		ИтоговыйСписокОтносительный.Добавить(СтрЗаменить(ОтносительныйПутьКФайлу, "/", "\"));
	КонецЦикла;
		
	ПараметрыКонфигурацииИсточник = РаботаСГитом.ПараметрыДляРаботыСКонфигурацией(БазаИсточник);
	ПараметрыКонфигурацииПриемник = РаботаСГитом.ПараметрыДляРаботыСКонфигурацией(БазаПриемник);
	
	КомандаЗапуска = РаботаСГитомКлиентСервер.КомандаДляЗапускаЗагрузкиОбъектовBICMD(ПараметрыВыполнения,
		ПараметрыКонфигурацииПриемник, ИтоговыйСписокОтносительный);
		
	ИмяФайлаЛога = ПараметрыВыполнения.РабочийКаталог + "\log.txt"; 
	РаботаСГитомКлиентСервер.ДобавитьВыводВФайл(КомандаЗапуска, ПараметрыВыполнения, ИмяФайлаЛога);
...
Функция ЭтоДополнительныеДанные(РазобранноеМетаданное) 
	Граница = РазобранноеМетаданное.ВГраница();
	Для Сч = 0 По Граница Цикл
		ТекущаяСтрока = РазобранноеМетаданное[Сч];
		Если ТекущаяСтрока = "Forms" Тогда //Папка формы?
			Если Сч + 2 > Граница Тогда
				Возврат Ложь;
			КонецЕсли;
			Если РазобранноеМетаданное[Сч + 2] = "Ext" Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
